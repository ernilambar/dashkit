...
async function handleAction( btn ) {
	...
	if ( ! endpointUrl ) {
		console.error(
			'Dashkit: data-endpoint-url is required on action buttons.'
		);
		return;
	}
		...
		switch ( field.type ) {
			...
			case 'select': {
				...
				( field.choices || [] ).forEach( function ( c ) {
					...
					const lbl =
						c !== null && typeof c === 'object' ? c.label : c;
		...
		fetch( REST + '/options', {
			...
			.then( ( data ) => {
				if ( data.success ) {
					...
					status.className =
						'dashkit-options-panel__status is-success';
					setTimeout( () => {
						status.textContent = '';
						status.className = 'dashkit-options-panel__status';
					}, 2500 );
...
function collectOptions( panel, schema ) {
	...
	schema.forEach( function ( field ) {
		switch ( field.type ) {
			...
			case 'toggle':
				out[ field.key ] =
					panel.querySelector( `[data-key="${ field.key }"]` )
						.checked === true;
				break;
			case 'radio':
			case 'buttonset': {
				const checked = panel.querySelector(
					`[data-key="${ field.key }"]:checked`
				);
			...
			case 'number':
				...
				break;
			default:
				out[ field.key ] = panel.querySelector(
					`[data-key="${ field.key }"]`
				).value;
		}
	} );
...
function initChart( canvas ) {
	...
	try {
		...
		console.error(
			'Dashkit Chart: invalid data JSON on canvas',
			canvas.id
		);
		return;
	}
...
document.addEventListener( 'DOMContentLoaded', function () {
	document
		.querySelectorAll( '.dashkit-options-panel' )
		.forEach( buildOptionsPanel );
